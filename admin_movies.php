...
function fetchTrailerFromTMDB($imdb_id, $fallbackTitle = '') {
    $tmdbKey = defined('TMDB_API_KEY') ? TMDB_API_KEY : (<?php
require_once 'config.php';
...
function fetchTrailerFromTMDB($imdb_id, $fallbackTitle = '') {
    ...
    if (empty($tmdbKey)) {
        return getTrailerUrl($fallbackTitle ?: $imdb_id);
    }
    ...
    if (!is_array($findData) || empty($findData['movie_results'][0]['id'])) {
        return getTrailerUrl($fallbackTitle ?: $imdb_id);
    }
    ...
    if (!is_array($videosData) || empty($videosData['results']) || !is_array($videosData['results'])) {
        return getTrailerUrl($fallbackTitle ?: $imdb_id);
    }
